...
def main():
    ...
    # 🔍 FIND EXISTING COMMENT (ROBUST)
    existing_comment = None
    existing_hash = None

    for comment in pr.get_issue_comments():
        logger.info("Inspecting existing comment id=%s", comment.id)

        if MARKER in comment.body or TITLE in comment.body:
            logger.info("Found existing security policy comment id=%s", comment.id)
